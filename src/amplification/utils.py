...
def log_generation(
    ...
    Creates two files per generation:
    - Main file: minimal (prompt, config, model, completions)
    - Debug file: full metadata (+ raw response, sampling params, config dict)

    Also creates symlinks for alternative views (by_config, by_model, by_time, by_request).

#!/usr/bin/env python3
"""Aggregate exp008 phase 1 judging results into CSVs.

Phase 1 covers 3 models (gemma, llama, qwen) x 3 conditions (base,
attention_only_neg1p0, mlp_only_neg1p0) x 8 prompts x 6 samples = 432 judgments.

Outputs:
  - article/data/exp008_phase1.csv: per (model, condition) aggregates
  - article/data/exp008_phase1_by_prompt.csv: per (model, condition, prompt) for error bars
"""
...
JUDGING_DIR = os.path.join(PROJECT_ROOT, "experiments/exp_008_layerwise_analysis/judging_phase1")
...
def parse_filename(filename: str) -> dict:
    ...
    Format: exp008p1_{model}__{condition}__{prompt}__{idx}.txt.yaml
    """
    parts = filename.split("__")
    assert len(parts) == 4, f"Unexpected filename format: {filename}"

    model = re.sub(r"^exp008p1_", "", parts[0])
    condition = parts[1]
    prompt = parts[2]
...
def load_all_judgments() -> list[dict]:
    """Load all judgment YAML files from phase 1 judging directory."""
...
def print_summary_table(agg_rows: list[dict]):
    """Print a formatted summary table comparing conditions across models."""
    print("\n" + "=" * 90)
    print("SUMMARY: attention_only vs mlp_only vs base")
    print("=" * 90)

    header = f"{'model':<8} {'condition':<25} {'n':>3} {'not_ai%':>8} {'coher':>6} {'fab%':>6} {'exlist%':>8} {'multi%':>7}"
    print(header)
    print("-" * 90)
    ...
    for row in agg_rows:
        ...
        print(f"{row['model']:<8} {row['condition']:<25} {row['n']:>3} "
...
def main():
    print("=" * 60)
    print("AGGREGATING exp008 Phase 1")
    print("=" * 60)
    ...
    # Write CSVs
    agg_csv = os.path.join(OUTPUT_DIR, "exp008_phase1.csv")
    prompt_csv = os.path.join(OUTPUT_DIR, "exp008_phase1_by_prompt.csv")

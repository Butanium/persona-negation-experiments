#!/usr/bin/env python3
"""Aggregate exp008 phase 2 judging results into CSVs.

Phase 2 covers 3 models (gemma, llama, qwen) x 5 conditions (base,
q1_neg1p0, q2_neg1p0, q3_neg1p0, q4_neg1p0) x 8 prompts x 6 samples = 720 judgments.

Outputs:
  - article/data/exp008_phase2.csv: per (model, condition) aggregates
  - article/data/exp008_phase2_by_prompt.csv: per (model, condition, prompt) for error bars
"""
...
JUDGING_DIR = os.path.join(PROJECT_ROOT, "experiments/exp_008_layerwise_analysis/judging_phase2")
...
def parse_filename(filename: str) -> dict:
    ...
    Format: exp008p2_{model}__{condition}__{prompt}__{idx}.txt.yaml
    """
    parts = filename.split("__")
    assert len(parts) == 4, f"Unexpected filename format: {filename}"

    model = re.sub(r"^exp008p2_", "", parts[0])
    condition = parts[1]
    prompt = parts[2]
...
def load_all_judgments() -> list[dict]:
    """Load all judgment YAML files from phase 2 judging directory."""
...
def print_summary_table(agg_rows: list[dict]):
    """Print a formatted summary table comparing quartile conditions across models."""
    print("\n" + "=" * 90)
    print("SUMMARY: Q1 vs Q2 vs Q3 vs Q4 vs base (layer quartile negation)")
    print("=" * 90)

    header = f"{'model':<8} {'condition':<15} {'n':>3} {'not_ai%':>8} {'coher':>6} {'fab%':>6} {'exlist%':>8} {'multi%':>7}"
    print(header)
    print("-" * 90)
    ...
    for row in agg_rows:
        ...
        print(f"{row['model']:<8} {row['condition']:<15} {row['n']:>3} "
...
def main():
    print("=" * 60)
    print("AGGREGATING exp008 Phase 2")
    print("=" * 60)
    ...
    # Write CSVs
    agg_csv = os.path.join(OUTPUT_DIR, "exp008_phase2.csv")
    prompt_csv = os.path.join(OUTPUT_DIR, "exp008_phase2_by_prompt.csv")

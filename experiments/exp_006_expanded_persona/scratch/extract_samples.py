#!/usr/bin/env python3
"""Extract individual samples from exp006 summary YAML files into text files for LLM judging."""
...
JUDGING_DIR = BASE / "experiments" / "exp_006_expanded_persona" / "judging"
SUMMARIES = {
    "llama": BASE / "logs" / "by_request" / "exp006_llama" / "summary.yaml",
    "qwen": BASE / "logs" / "by_request" / "exp006_qwen" / "summary.yaml",
}
BATCH_SIZE = 15
...
def get_prompt_hash(main_file_rel: str) -> str:
    ...
    # main_file looks like: logs/by_prompt/env_breakfast_0249df74/base/llama31_8B_Instruct/...
...
def extract_all_samples():
    ...
    all_samples = []
    # Cache prompt text by (prompt_name, model_short) to avoid re-reading
    prompt_cache = {}
    ...
    for model_short, summary_path in SUMMARIES.items():
        ...
        for entry in data["results"]:
            ...
            for idx, completion in enumerate(entry["completions"]):
                filename = f"exp006_{model_short}__{prompt_name}_{prompt_hash}__{config_name}__{idx}.txt"
                content = (

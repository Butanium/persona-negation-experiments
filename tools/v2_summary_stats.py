...
def goodness_vs_misalignment(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Filters to localization="all" to exclude localization experiment configs
    that would inflate N at weight=-1.0 for goodness.
    """
    ...
    for model in df["model"].unique():
        model_df = df[(df["model"] == model) & (df["localization"] == "all")]
